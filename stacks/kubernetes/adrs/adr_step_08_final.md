...
1. **externaliser l'intégralité de la configuration Nginx** dans un ConfigMap afin de piloter la configuration sans reconstruire l'image (HTTP + HTTPS) ;
...
On crée un ConfigMap `nginx-config` (namespace `dev`) contenant l'intégralité de la configuration Nginx dans une clé `default.conf`, avec deux blocs `server` : un en HTTP (port 80) et un en HTTPS (port 443, certificats montés depuis le Secret). Il est marqué `immutable: true`.
...
En contrepartie, toute évolution de la configuration impose de **supprimer puis recréer** le ConfigMap : les pods doivent être redémarrés pour consommer la nouvelle version.
...
Le vrai Secret n'est pas commité. Des fichiers d'exemple placeholders sont fournis et les vrais Secrets doivent être générés hors bande avant déploiement.
...
Ce choix tranche en faveur du découplage : le conteneur `nginx` reste minimal et n'embarque aucune logique de rotation. La rotation reste volontairement simple à cette étape ; la compression, la rétention longue et l'envoi vers un collecteur centralisé sont hors scope.

### 2.4. Montage de la configuration via `subPath`

Le fichier `default.conf` est monté sur `/etc/nginx/conf.d/default.conf` avec `subPath: default.conf` et `readOnly: true`. Ce chemin est le standard accepté par l'image Nginx officielle pour les configurations additionnelles. Le `subPath` évite de masquer tout le répertoire `/etc/nginx/conf.d` par un simple fichier, et le `readOnly` garantit qu'aucun conteneur ne peut altérer la configuration servie. Cette combinaison est cohérente avec l'immuabilité choisie en 2.1.
...
* **Immuabilité + `subPath` :** modifier la configuration exige de supprimer/recréer le ConfigMap **et** de redémarrer les pods. Aucune propagation « à chaud » n'est possible.
...
* **DaemonSet de collecte des logs en root :** le DaemonSet `log-pod-ds` monte `/var/log/pods` en `hostPath` et tourne volontairement en root. Cette exception est conservée conformément à l'ADR précédente, afin de pouvoir lire les logs de tous les pods dans cet environnement K3s mono-nœud. Elle devra être revue si le cluster devient multi-tenant.
...
* **Bug rencontré – cache kubelet sur K3s :** lors du test *break-it*, après suppression du ConfigMap `nginx-config` (`kubectl delete cm -n dev nginx-config`), les pods recréés via `k rollout restart` ou `k delete pods` ont démarré sans erreur. Le `describe pod` indiquait pourtant `Optional: false` sur le volume `nginx-config`, et `kubectl get cm -n dev` confirmait l'absence de l'objet. Aucun événement `FailedMount` n'a été généré. En inspectant le pod, la configuration servie était encore l'ancienne version de la ConfigMap, et la readiness probe `/hello` passait. Nous avons reproduit ce comportement plusieurs fois : suppression, recréation des pods, redémarrage complet de la stack (`k delete -k infra/ && k apply -k infra/`) ; à chaque fois les pods démarraient `3/3`.
    ...
    * **Impact sur la validation** : le scénario `break-it` ne peut pas valider l'erreur attendue (`ContainerCreating` + `FailedMount`) tant que le cache kubelet n'est pas purgé. Le script `tests/step_08_validation.sh` marque donc ce cas comme **NON_CONCLUANT** au lieu de `FAIL`, sauf si un redémarrage explicite de K3s est réalisé avant l'exécution. Ce comportement doit être pris en compte dans les scripts de validation et la documentation de debug.
